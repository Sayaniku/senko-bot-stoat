...
module.exports = {
    name: 'kick',
    aliases: ['k'],
    description: 'Exclure un utilisateur du serveur.',
    ...
    execute: async (client, message, args) => {
        ...
        const hasPermission = isOwner || member.hasPermission(message.channel, "KickMembers");

        if (!hasPermission) {
            return await message.reply("❌ Vous devez avoir la permission d'expulser.");
        }
        ...
        try {
            ...
            if (targetMember.id.user === server.ownerId) {
                return await message.reply("❌ Vous ne pouvez pas expulser le propriétaire du serveur.");
            }
            ...
            if (targetRanking <= memberRanking) {
                return await message.reply("❌ Vous ne pouvez pas expulser quelqu'un avec un rôle égal ou supérieur au vôtre.");
            }

            if (targetRanking <= botRanking) {
                return await message.reply("❌ Je ne peux pas expulser quelqu'un avec un rôle égal ou supérieur au mien.");
            }
            ...
            // Réponse
            await message.reply(`✅ ${userDisplay} a été expulsé.\nRaison: ${reason}`);
                    ...
                    if (logChannelId) {
                        ...
                        if (logChannel) {
                            const embed = new SympactEmbedBuilder()
                                .setTitle('🦊 Renard expulsé')
                                .setDescription(`${userDisplay} a été expulsé par ${message.author}`)
                                .setColor("#FF0000")
                                .build();
